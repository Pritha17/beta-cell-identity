...
namespace LSODA_algo {
    ...
    */
    void LSODA::lsoda(LSODA_ODE_SYSTEM_TYPE f, const size_t neq, vector<double> &y,
                      ...
                      int jt, int *iworks, double *rworks,
                      ...
                      int aktp_signal, int foxo1_signal, int gsk3_signal)
    {
        ...
        */
        if (*istate == 1)
        {
            ...
            (*f)(*t, &y[1], &yh_[2][1], data, metblsm_par, ros_signal, insulin_signal, aktp_signal, foxo1_signal, gsk3_signal);
            nfe = 1;
        ...
        */
        while (1)
        {
            ...
            /* Call stoda */
            stoda(neq, y, f, data, metblsm_par, ros_signal, insulin_signal, aktp_signal, foxo1_signal, gsk3_signal);
            if (kflag == 0)
            {
                /*
                   Block f.
                      ...
                      int aktp_signal, int foxo1_signal, int gsk3_signal)
    {
        ...
        double del1 = 0.0, delp = 0.0, dsm = 0.0, dup = 0.0, exup = 0.0, r = 0.0,
        ...
        */
        while (1)
        {
            while (1)
            {
                ...
                correction(neq, y, f, &corflag, pnorm, &del1, &delp, &told, &ncf, &rh, &m,
                           data, metblsm_par, ros_signal, insulin_signal, aktp_signal, foxo1_signal, gsk3_signal);
                if (corflag == 0)
                    break;
                if (corflag == 1)
                {
            ...
            else
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        for (i = 1; i <= n; i++)
                            y[i] = yh_[1][i];
                        (*f)(tn_, &y[1], &savf[1], data, metblsm_par, ros_signal, insulin_signal, aktp_signal, foxo1_signal, gsk3_signal);
                        nfe++;
                        for (i = 1; i <= n; i++)
                            yh_[2][i] = h_ * savf[i];
                        ipup = miter;
        ...
        */
        ...
        if (miter == 2)
        {
            ...
            for (j = 1; j <= n; j++)
            {
                ...
                y[j] += r;
                fac = -hl0 / r;
                (*f)(tn_, &y[1], &acor[1], data, metblsm_par, ros_signal, insulin_signal, aktp_signal, foxo1_signal, gsk3_signal);
                for (i = 1; i <= n; i++)
    ...
    */
    void LSODA::correction(const size_t neq, vector<double> &y,
                           ...
                           double *del1, double *delp, double *told, size_t *ncf,
                           double *rh, size_t *m, vector<double> data, vector<double> metblsm_par,
        ...
        */
        while (1)
        {
            if (*m == 0)
            {
                if (ipup > 0)
                {
                    prja(neq, y, f, data, metblsm_par, ros_signal, insulin_signal, aktp_signal, foxo1_signal, gsk3_signal);
                    ipup = 0;
                    rc = 1.;
                    nslp = nst;
                    crate = 0.7;
            ...
            */
            ...
            if (*m != 0 || meth_ != 1)
            {
                ...
                dcon = *del1 * min(1., 1.5 * crate) / (tesco[nq][2] * conit);
                if (dcon <= 1.)
                {
    ...
    */
                            ...
                            )
    {
        ...
        lsoda(f, neq, yout, t, tout, itask, istate, iopt, jt, iworks, rworks, data, metblsm_par, ros_signal,
        insulin_signal, aktp_signal, foxo1_signal, gsk3_signal);
    }
}
