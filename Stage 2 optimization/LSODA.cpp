...
namespace LSODA_algo {
                      ...
                      vector<double> data, vector<double> metblsm_par, int ros_signal, int insulin_signal)
    {
        ...
        */
        if (*istate == 1)
        {
            ...
            (*f)(*t, &y[1], &yh_[2][1], data, metblsm_par, ros_signal, insulin_signal);
            nfe = 1;
        ...
        */
        while (1)
        {
            ...
            /* Call stoda */
            stoda(neq, y, f, data, metblsm_par, ros_signal, insulin_signal);
            if (kflag == 0)
            {
                /*
                   Block f.
        ...
        while (1)
        {
            while (1)
            {
                ...
                correction(neq, y, f, &corflag, pnorm, &del1, &delp, &told, &ncf, &rh, &m,
                           data, metblsm_par, ros_signal, insulin_signal);
                if (corflag == 0)
                    break;
                if (corflag == 1)
                {
            ...
            else
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        for (i = 1; i <= n; i++)
                            y[i] = yh_[1][i];
                        (*f)(tn_, &y[1], &savf[1], data, metblsm_par, ros_signal, insulin_signal);
                        nfe++;
                        for (i = 1; i <= n; i++)
                            yh_[2][i] = h_ * savf[i];
                        ipup = miter;
        ...
        */
        ...
        if (miter == 2)
        {
            ...
            for (j = 1; j <= n; j++)
            {
                ...
                y[j] += r;
                fac = -hl0 / r;
                (*f)(tn_, &y[1], &acor[1], data, metblsm_par, ros_signal, insulin_signal);
                for (i = 1; i <= n; i++)
    ...
    */
    void LSODA::correction(const size_t neq, vector<double> &y,
                           ...
                           int ros_signal, int insulin_signal)
    {
        ...
        */
        while (1)
        {
            if (*m == 0)
            {
                if (ipup > 0)
                {
                    prja(neq, y, f, data, metblsm_par, ros_signal, insulin_signal);
                    ipup = 0;
                    rc = 1.;
                    nslp = nst;
                    crate = 0.7;
    ...
    */
                            ...
                            )
    {
        ...
        lsoda(f, neq, yout, t, tout, itask, istate, iopt, jt, iworks, rworks, data, metblsm_par, ros_signal, insulin_signal);
    }
}
